...
class Section
	...
	def initialize(num, comp, loc, time, ins, ses, top)
		@class_number=num
		@component=comp
		@location=loc
		@times=time
...
class Web_Scraping
	...
	def scraper()
		...
		for i in 0..total_courses.count-1
			...
			@courses.push(Course.new(heading))
			...
			for j in 1..total_rows_for_course-1
				if subsections.at(i).css('tr').at(j).count>0
					...
					@courses[i].section.push(Section.new(num, comp, loc, time, ins, ses, top))
					CourseSection.create(Name: heading, ClassNumber: num, Component: comp, Location: loc, Times: time, Instructor: ins, Session: ses, Topic: top)
				end
			end
		end
	end
